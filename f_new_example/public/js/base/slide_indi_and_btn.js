...
$.fn.myIndiSlide = function(jsonList, imgList){
  ...
  <button class="prev" type="button"><span>이전내용보기</span></button></div>')
  ...
  indicator.find('li').css({display:'inline-block', marginRight:'1rem'});
  indicator.find('a').css({display:'block', width:'26px', height:'26px', backgroundColor:'#ced', borderRadius:'100%'});
  ...
  /* let indiDefault = indicator.children('li').eq(0).css('backgroundColor');
  if(indiIndex.hasClass('action')){
    indiIndex.children('a').css({backgroundColor:'#fec'});
  }else{
    indiIndex.siblings().css({backgroundColor:indiDefault});
  } */

  $('head').append('<style class="myStyle"></style>');
  $('head').find('.myStyle').append('.indicator li.action > a{background-color:#def !important;}');
  // -----------------------------------------------------
  // indicator 클릭 / focus
